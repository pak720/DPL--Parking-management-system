...
# Cache CLAHE object — tránh khởi tạo lại mỗi frame
_clahe_plate = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
_clahe_enhance = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
...
def detect_face(frame):
    results = face_yolo(frame, conf=0.5, half=use_half, verbose=False)[0]
...
def detect_clothes(frame):
    results = clothes_yolo(frame, conf=0.5, half=use_half, verbose=False)[0]
    if len(results.boxes) == 0:
        return None
...
def detect_helmet(frame):
    results = helmet_model(frame, conf=0.5, half=use_half, verbose=False)[0]
    if len(results.boxes) == 0:
        return "no"
    return "yes"
...
def detect_helmet_color(frame):
    results = helmet_color_model(frame, conf=0.5, half=use_half, verbose=False)[0]
    if len(results.boxes) == 0:
        return "unknown"
...
def build_plate_variants(frame):
    # Chỉ dùng 1 variant đã enhance — giảm từ 3×2=6 xuống 1×1=1 lần inference
...
def enhance_plate_image(frame):
    ...
    """
    # 1. Áp dụng CLAHE (dùng object cache, không tạo mới mỗi frame)
    lab = cv2.cvtColor(frame, cv2.COLOR_BGR2LAB)
    l, a, b = cv2.split(lab)
    l = _clahe_plate.apply(l)
...
def detect_license_plate(frame):
    ...
    try:
        ...
        # chạy 1 size duy nhất để giảm lag (từ 2 sizes xuống 1)
        detect_sizes = [640]
